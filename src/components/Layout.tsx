...
export function Layout({ children, session }: any) {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-900 overflow-hidden">
      <nav className="bg-gray-800 shadow-lg border-b border-gray-700 flex-shrink-0">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
          <div className="flex justify-between items-center h-14 sm:h-16">
            <div className="flex items-center space-x-2 sm:space-x-4">
              <h1 
                className="text-lg sm:text-xl font-bold text-white cursor-pointer hover:text-blue-400 transition"
                onClick={() => router.push('/dashboard')}
              >
                MentorConnect
              </h1>
              <span className="text-xs bg-blue-600 text-white px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full">Beta</span>
            </div>
